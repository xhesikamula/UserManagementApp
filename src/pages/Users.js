            ...
            <div className="container mx-auto">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                    <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-2">
                        <input type="search" placeholder="Search by name or email" className="w-[400px] border border-[#3a4920] px-6 py-2 mr-3" value={search} onChange={(e) => setSearch(e.target.value)}/>
                        <button onClick={() => setSearch("")} className="underline">Clear</button>
                    </div>
                    <button
                        onClick={toggleSort}
